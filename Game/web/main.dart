...
class Game {
  ...
  void init() {
    ...
    playerCube = new THREE.Mesh(new THREE.CubeGeometry(10.0, 20.0, 10.0),
        new THREE.MeshBasicMaterial(color: 0xfaff00));
    ...
    controls.lookVertical =
        false; // Temporary solution; play on flat surfaces only
  ...
  void generateWorld() {
    Levels level1 = new Levels(this.levelBlockWidth, this.levelBlockLength,
        this.blockHeight, this.blockWidth, this.blockDepth,
        this.wallBlockHeight);
    ...
    for (int i = 0; i < ground.ground.length - 1; i++) {
      ...
      var floorBlock = new THREE.Mesh(new THREE.CubeGeometry(
              block.getWidth().toDouble(), block.getHeight().toDouble(),
              block.getDepth().toDouble()),
          new THREE.MeshLambertMaterial(color: block.getColor()));
    ...
    for (int i = 0; i < wall.wall.length - 1; i++) {
      ...
      var wallBlock = new THREE.Mesh(new THREE.CubeGeometry(
          block.getWidth().toDouble(), block.getHeight().toDouble(), block
              .getDepth()
              .toDouble()), new THREE.MeshPhongMaterial(color: 0xc2d1d3));
    ...
    var sideWall1 = new THREE.Mesh(
        ...
        new THREE.MeshLambertMaterial(color: 0xe08b3c));
    ...
    sideWall1.position.setValues(this.planeWidth / 2 - this.blockWidth / 2,
        this.planeHeight / 2, -this.blockWidth / 2);
    scene.add(sideWall1);

    sideWall2.doubleSided = true;
    sideWall2.position.setValues(this.planeWidth / 2 - this.blockWidth / 2,
        this.planeHeight / 2,
        this.levelBlockLength * this.blockDepth - this.blockWidth / 2);
    sideWall2.rotation.x = THREE.degToRad(180);
    scene.add(sideWall2);

    sideWall3.position.setValues(0.0 - this.blockWidth / 2,
        this.planeHeight / 2,
        this.levelBlockLength * this.blockDepth / 2 - this.blockWidth / 2);
    sideWall3.rotation.y = THREE.degToRad(90);
    scene.add(sideWall3);

    sideWall4.position.setValues(this.planeWidth - this.blockWidth / 2,
        this.planeHeight / 2,
        this.levelBlockLength * this.blockDepth / 2 - this.blockWidth / 2);
  ...
  void generateEnemies() {
    ...
    enemies = new Enemies(numOfEnemies, levelBlockWidth * blockWidth,
        levelBlockLength * blockDepth, blockHeight);
  }
  ...
  void setLevelLight() {
    light = new THREE.DirectionalLight(0xffffff, 0.2)
      ...
      ..position.setValues(levelBlockWidth.toDouble() * blockWidth, 400.0,
          (levelBlockWidth * blockWidth) / 2.0)
      ..shadowCameraFar = 600
    ...
    light2 = new THREE.DirectionalLight(0xffffff, 0.2)
      ...
      ..shadowMapHeight = levelBlockWidth * blockWidth
      ..shadowMapWidth = levelBlockWidth * blockWidth
      ..position.setValues(levelBlockWidth.toDouble(), 400.0,
          (levelBlockWidth * blockWidth) / 2.0)
      ..shadowCameraFar = 600
      ..shadowCameraLeft = -(levelBlockWidth * blockWidth) / 2
      ..shadowCameraRight = (levelBlockWidth * blockWidth) / 2
    ...
    for (int i = 0; i < number; i++) {
      ...
      loader.load('obj/man.obj').then((object) {
        object.children.forEach((e) {
          if (e is THREE.Mesh) {
            ((e as THREE.Mesh).material as THREE.MeshLambertMaterial).map =
                texture;
          }
        });
        object.name = "enemi";
  ...
  void animate(num time) {
    ...
    if (this.controls != null && ray == null) {
      Vector3 vector = this.controls.target
          .clone()
          .sub(this.controls.object.position)
          .normalize();
      ray = new List<THREE.Ray>();
      ray.add(new THREE.Ray(this.controls.object.position,
